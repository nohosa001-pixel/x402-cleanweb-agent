...
### MCP 도구 목록:
1. `get_payment_info()`: 결제 수신 지갑, 네트워크, 금액 안내
2. `fetch_clean_web_content(url, payment_tx_hash)`:
...
```json
{
  ...
  "x402": {
    ...
    "amount_raw": "10000",
    "decimals": 6,
    "instructions": "Transfer 0.01 USDC to 0x742d35Cc6634C0532925a3b844Bc454e4438f44e on Polygon, then include header 'X-Payment-Tx: <TX_HASH>'."
  }
}
```
...
```bash
curl -H "X-Payment-Tx: 0xYourPolygonTxHash..." \
     "http://localhost:8000/api/v1/clean-web?url=https://example.com"
```
**응답:**
```json
{
