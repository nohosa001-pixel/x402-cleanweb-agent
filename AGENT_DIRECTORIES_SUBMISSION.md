# 📡 AI Agent Directory Submission & Indexing Guide

This guide contains pre-formatted submission text to list **x402-cleanweb-agent** in the top global AI agent & MCP directories.

---

## 1. 🌟 Awesome-MCP-Servers (GitHub PR)
- **Repository**: [https://github.com/punkpeye/awesome-mcp-servers](https://github.com/punkpeye/awesome-mcp-servers)
- **Section**: `Web Scraping / Data Extraction` or `Finance & Web3`
- **Markdown line to add**:
```markdown
- [x402 CleanWeb Agent](https://github.com/nohosa001-pixel/x402-cleanweb-agent) 🪙 - Autonomous Web3 x402 micropayment MCP suite on Polygon for clean web markdown, batch scraping, YouTube transcripts, and PDF papers.
```

---

## 2. 🦙 Glama.ai MCP Directory
- **Submit URL**: [https://glama.ai/mcp/servers](https://glama.ai/mcp/servers)
- **Server Name**: `polygon-x402-cleanweb`
- **GitHub Repository**: `https://github.com/nohosa001-pixel/x402-cleanweb-agent`
- **PyPI Package**: `x402-cleanweb-agent`
- **Short Description**:
> Autonomous Web3 x402 Micropayment MCP suite for AI-ready Clean Web, YouTube Transcripts, and PDF Research Papers on Polygon Mainnet.
- **Install Command**:
```bash
uvx x402-cleanweb-agent
```

---

## 3. 🤖 CrewAI / LangChain Community
- **Package Name**: `x402-cleanweb-agent`
- **Import**: `from agent_tools import get_x402_agent_tools, X402AgentToolkit`
- **Docs**: `https://x402-cleanweb-agent-212942243360.asia-northeast3.run.app/llms.txt`

---

## 4. 🌐 Machine Indexing Endpoints (Live GCP Cloud Run)
- **Live Endpoint**: `https://x402-cleanweb-agent-212942243360.asia-northeast3.run.app`
- **LLM Context (`/llms.txt`)**: `https://x402-cleanweb-agent-212942243360.asia-northeast3.run.app/llms.txt`
- **MCP Server Card**: `https://x402-cleanweb-agent-212942243360.asia-northeast3.run.app/.well-known/mcp.json`
- **Agent Discovery**: `https://x402-cleanweb-agent-212942243360.asia-northeast3.run.app/.well-known/agent.json`
- **Pricing Catalog**: `https://x402-cleanweb-agent-212942243360.asia-northeast3.run.app/api/v1/agent/pricing-catalog`
- **PyPI Package**: `https://pypi.org/project/x402-cleanweb-agent/`
