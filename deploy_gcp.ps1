...
gcloud run deploy x402-cleanweb-agent `
    ...
    --cpu 1 `
    --min-instances 1 `
    --set-env-vars="SERVER_WALLET_ADDRESS=0x255F9991233f86B29dB847c8d5b8CB9915e80dCf,PAYMENT_AMOUNT_USDC=0.01,POLYGON_RPC_URL=https://polygon-bor-rpc.publicnode.com,GEMINI_API_KEY=$($env:GEMINI_API_KEY),GEMINI_MODEL=gemini-3.6-flash,SECURITY_GATE_URL=https://agent-security-gate-x402-212942243360.asia-northeast3.run.app,AGENT_PAYMENT_VAULT_ADDRESS=$($env:AGENT_PAYMENT_VAULT_ADDRESS),CLEANWEB_ORACLE_CONTRACT_ADDRESS=$($env:CLEANWEB_ORACLE_CONTRACT_ADDRESS),CLEANWEB_ORACLE_VERIFIER_ADDRESS=$($env:CLEANWEB_ORACLE_VERIFIER_ADDRESS),BASE_AGENT_PAYMENT_VAULT_ADDRESS=$($env:BASE_AGENT_PAYMENT_VAULT_ADDRESS),BASE_CLEANWEB_ORACLE_CONTRACT_ADDRESS=$($env:BASE_CLEANWEB_ORACLE_CONTRACT_ADDRESS),BASE_CLEANWEB_ORACLE_VERIFIER_ADDRESS=$($env:BASE_CLEANWEB_ORACLE_VERIFIER_ADDRESS),ARBITRUM_AGENT_PAYMENT_VAULT_ADDRESS=$($env:ARBITRUM_AGENT_PAYMENT_VAULT_ADDRESS),ARBITRUM_CLEANWEB_ORACLE_CONTRACT_ADDRESS=$($env:ARBITRUM_CLEANWEB_ORACLE_CONTRACT_ADDRESS),ARBITRUM_CLEANWEB_ORACLE_VERIFIER_ADDRESS=$($env:ARBITRUM_CLEANWEB_ORACLE_VERIFIER_ADDRESS),ORACLE_SIGNER_PRIVATE_KEY=$($env:ORACLE_SIGNER_PRIVATE_KEY),SERVER_PRIVATE_KEY=$($env:SERVER_PRIVATE_KEY),FREE_TRIAL_LIMIT=$($env:FREE_TRIAL_LIMIT)" `
    --quiet
